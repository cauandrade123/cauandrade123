<div align="center">

# ⚡ Cauã Andrade Condé
### Back-End Developer • Node.js • Cloud • Databases

<br/>

![Profile Views](https://komarev.com/ghpvc/?username=cauandrade123&color=blueviolet&style=flat-square&label=Visualizações)

---

## 🚀 Sobre mim

Desenvolvedor Back-End focado na construção de **APIs escaláveis**,
**arquitetura limpa** e sistemas profissionais.

Trabalho com Node.js/Express no dia a dia, mas transito por Python, bancos relacionais
(MySQL/PostgreSQL) e containerização com Docker. Minha prioridade é entregar
soluções robustas, seguindo os melhores padrões da indústria — do design da API
até o deploy.

---

## 🛠️ Tecnologias & Stacks

<br/>

<p align="center">
  <a href="https://skillicons.dev">
    <img src="https://skillicons.dev/icons?i=js,ts,nodejs,express,python,mysql,postgres,react,docker,aws,git,postman&theme=dark" height="120" alt="Minhas Stacks" />
  </a>
</p>

<br/>

### 🧠 Princípios de Engenharia

**Clean Code • SOLID • Separation of Concerns**
**Escalabilidade • Segurança • Arquitetura em Camadas**

### 🧰 Ferramentas do dia a dia

`Express.js` `Sequelize / Knex` `Docker & Docker Compose` `Postman` `Git & GitHub` `AWS`

---

## 📊 Estatísticas do GitHub

<br/>

<img height="210em" src="https://github-readme-stats-sigma-five.vercel.app/api?username=cauandrade123&show_icons=true&theme=tokyonight&include_all_commits=true&count_private=true&hide_border=true" alt="GitHub Stats" />
<br/>
<img height="210em" src="https://github-readme-stats-sigma-five.vercel.app/api/top-langs/?username=cauandrade123&layout=compact&langs_count=8&theme=tokyonight&hide_border=true" alt="Top Linguagens" />

<br/>

![GitHub Streak](https://github-readme-streak-stats.herokuapp.com/?user=cauandrade123&theme=tokyonight&hide_border=true)

---

## 📫 Vamos nos conectar?

<br/>

<a href="mailto:cauaandradeconde@gmail.com">
  <img src="https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white" height="40" alt="Gmail" />
</a>
&nbsp;
<a href="https://www.linkedin.com/in/SEU-LINKEDIN-AQUI" target="_blank">
  <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" height="40" alt="LinkedIn" />
</a>
&nbsp;
<a href="https://cauaandradeconde.com.br" target="_blank">
  <img src="https://img.shields.io/badge/Portfolio-000000?style=for-the-badge&logo=vercel&logoColor=white" height="40" alt="Portfolio" />
</a>

---

<sub>Eficiência, Escala e Código Limpo.</sub>

</div>
